...
.section-title { font-size:17px; font-weight:700; color:#e0e0e0; margin:20px 0 10px 0; paddin
...
gc = data['Ghi chu'].astype(str).str.upper() if 'Ghi chu' in data.columns else pd.Serie
...
n_ct nhom_h = risk_table[risk_table['Ten_KH']==kh_sel]['Ten_nhom_KH'].values[0] if (not risk_table
kv_h = risk_table[risk_table['Ten_KH']==kh_sel]['Khu_vuc'].values[0] if (not risk_table.
...
line=dict(color='#f0a500',width=2), marker=dict(size=7)), row
...
tra_c = [c for c in ['So chung tu','Ngay chung tu','Ten hang','Khoi luong','Thanh tie
...
sh2 = [c for c in ['So chung tu','Ngay chung tu','Ten hang','Gia ban','Thanh tien ban
...
title='Top SP x Thang', color_continuous_scale='Blues', fig_hm.update_layout(height=max(380, len(top_n)*24))
...
mc[3].metric('GT tra hang VND','{:,.0f}'.format(abs(df_tra['Thanh tien ban'].sum()) if no
st.markdown('''
<div class="info-box">
...
po_c = [c for c in ['So chung tu','Ngay chung tu','Ten hang','Thanh tien ban','Gh
...
if tl_tra > 10:
risks.append(('high', 'Ty le hang tra: <b>{:.1f}%</b> ({:,.0f} VND) - rui ro elif tl_tra > 3:
