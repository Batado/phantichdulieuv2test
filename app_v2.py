...
COL_ALIASES = {
    "Ten khach hang": ["Ten khach hang", "Ten KH", "Khach hang", "Ten khach", "customer_name"],
    ...
    "Bien so xe": ["So xe", "Bien so xe", "plate_no"],
...
def load_all(file_data):
    ...
    loai = df["Ma nhom KH"].astype(str).str.upper() if "Ma nhom KH" in df.columns else pd.Series("", index=df.index)
    df["Loai GD"] = "Xuat ban"
...
def build_risk_table(file_data):
    ...
    for kh_name, grp in df.groupby("Ten khach hang"):
        if not kh_name or str(kh_name) in ("", "nan"):
            continue
        sc = compute_risk(grp)
        nhom = grp["Ten nhom KH"].mode()[0] if "Ten nhom KH" in grp.columns and not grp["Ten nhom KH"].eq("").all() else ""
        kv   = grp["Khu vuc"].mode()[0] if "Khu vuc" in grp.columns and not grp["Khu vuc"].eq("").all() else ""
        rows.append({
            "Ten KH":    str(kh_name),
...
if has_kv:
    kv_opts = sorted([x for x in df_after_nhom["Khu vuc"].unique() if x not in ("", "nan", "None")])
else:
    kv_opts = []
...
kh_info = risk_table[risk_table[“Ten KH”] == kh].iloc[0] if not risk_table.empty and kh in risk_table[“Ten KH”].values else None
sc_kh   = int(kh_info[“Diem rui ro”]) if kh_info is not None else 0
lv_kh   = kh_info[“Muc rui ro”] if kh_info is not None else “?”
...
if len(df_ln) >= 2:
    ...
    if not anom.empty:
        for _, r in anom.iterrows():
            st.markdown(
                '<div class="risk-medium">Thang <b>{}</b>: Bien LN = <b>{:.1f}%</b> (TB={:.1f}%) - kha nang co chiet khau/chinh sach</div>'.format(
...
if not df_tra.empty:
    ...
    show_cols = [c for c in ["So chung tu","Ngay chung tu","Ten hang","Khoi luong","Thanh tien ban","Ghi chu"] if c in df_tra.columns]
...
with st.expander("Chi tiet gia ban tung giao dich"):
    show2 = [c for c in ["So chung tu","Ngay chung tu","Ten hang","Gia ban","Don gia quy doi",
                           "Thanh tien ban","Loi nhuan","Ghi chu"] if c in df_ban.columns]
    st.dataframe(df_ban[show2].sort_values("Ngay chung tu"), use_container_width=True, hide_index=True)
```
...
```
gc_s    = df["Ghi chu"].astype(str).str.upper() if "Ghi chu" in df.columns else pd.Series("", index=df.index)
...
mc[3].metric("GT tra hang (VND)", "{:,.0f}".format(abs(df_tra2["Thanh tien ban"].sum()) if not df_tra2.empty else 0))
...
if not df_po.empty:
    with st.expander("{} don co PO / ma du an".format(len(df_po))):
        show_po = [c for c in ["So chung tu","Ngay chung tu","Ten hang","Thanh tien ban","Ghi chu"] if c in df_po.columns]
...
st.markdown('<div class="section-title">Danh gia rui ro tong hop</div>', unsafe_allow_html=True)
...
if tl_tra > 10:
    score += 30
    risks.append(("high",   "Ty le hang tra: <b>{:.1f}%</b> ({:,.0f} VND) - rui ro cao".format(tl_tra, tong_tra)))
elif tl_tra > 3:
    score += 15
...
if bien < 5:
    score += 25
    risks.append(("high",   "Bien LN rat thap: <b>{:.1f}%</b> - ban duoi gia hoac chiet khau lon".format(bien)))
elif bien < 15:
    score += 10
...
if "Noi giao hang" in df_ban.columns:
    ...
    if n_noi >= 5:
        score += 10
        risks.append(("medium", "Giao hang toi <b>{}</b> dia diem - cong no phan tan".format(n_noi)))
    else:
...
if n_thang_mua / n_range < 0.5:
    score += 10
    risks.append(("medium", "Mua chi <b>{}/{}</b> thang - khong deu, phu thuoc du an".format(n_thang_mua, n_range)))
else:
...
st.markdown(
    ...
    '<div style="background:{};width:{}%;height:100%;border-radius:6px;"></div></div>'.format(sc_col2, pct_b),
    unsafe_allow_html=True
)
...
if not risk_table.empty:
    ...
    def make_badge(row):
        sc2 = row["Diem rui ro"]
        lv2 = row["Muc rui ro"]
        if sc2 >= 50:   return '<span class="risk-badge-cao">{} - {}pt</span>'.format(lv2, sc2)
        elif sc2 >= 25: return '<span class="risk-badge-tb">{} - {}pt</span>'.format(lv2, sc2)
        else:           return '<span class="risk-badge-thap">{} - {}pt</span>'.format(lv2, sc2)
    ...
    # Summary by Phong KD
    if "Ten nhom KH" in rt_display.columns and rt_display["Ten nhom KH"].replace("","nan").nunique() > 1:
        st.markdown('<div class="section-title">Tong hop rui ro theo Phong KD</div>', unsafe_allow_html=True)
    ...
    # Summary by Khu vuc
    if "Khu vuc" in rt_display.columns and rt_display["Khu vuc"].replace("","nan").nunique() > 1:
        st.markdown('<div class="section-title">Tong hop rui ro theo Khu vuc</div>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="section-title">Chi tiet toan bo khach hang</div>', unsafe_allow_html=True)
    rt_show = rt_display[["Ten KH","Ten nhom KH","Khu vuc","Diem rui ro","Muc rui ro"]].copy()
